 ...
 * 2023 Beaver (GEGL ZZStrokeBevelImageOverlay a hidden operation fork of drop shadow to give GEGL Effects outline the ability to add a bevel, image file overlay and more. May 16 23 it now gives GEGL Effects drop shadow a blurred image file overlay. May 17 2023 it now has an aura seed option for shadow glow )
  2024 Beaver, Zzstrokebevelimage fork without image file overlay 
  */
...
property_int (stroke, _("Grow radius"), 12)
  value_range   (0, 300)
  ui_range      (0, 100)
  ui_steps      (1, 5)
  ui_gamma      (1.5)
...
#define GEGL_OP_META
#define GEGL_OP_NAME     zzstrokebevel299
#define GEGL_OP_C_SOURCE zzstrokebevel299.c
...
*/
 ...
 if (o->enableaura)
  {
  gegl_node_link_many (state->input, state->median, state->cubismglow, state->gaussian, state->move,  state->ontop,  state->nop2, multiplyge, crop, state->output, NULL);
  ...
  {
  gegl_node_link_many (state->input, state->median,  state->gaussian, state->move,  state->ontop,  state->nop2, multiplyge, crop, state->output, NULL);
  gegl_node_link_many (state->nop2, state->bevel, state->opacitybevel,  NULL);
  gegl_node_connect (multiplyge, "aux", state->opacitybevel, "output");
  gegl_node_link_many (state->color,  state->opacity, NULL);
  gegl_node_connect (state->ontop, "aux", state->opacity, "output");
...
gegl_op_class_init (GeglOpClass *klass)
{
  ...
  gegl_operation_class_set_keys (operation_class,
    "name",        "lb:zzstrokebevel299",
    "title",       _("Hidden Operation to outline, shadow, glow in GE (no crash 2.99.19)"),
    "categories",  "hidden",
    "reference-hash", "33234v25str2ac299",
    "description", _("Hidden Operation for GEGL Effects - This is a rough reimagination of Gimp's drop shadow filter with many new things and technical options that drop shadow was not capable of doing. GEGL Effects no longer calls Gimp's GEGL drop shadow filter. It uses this under the hood. (2.99 friendly build)"
                     ""),
    NULL);
}
