...
enum_start (gegl_blend_mode_typeglowstick299)
...
property_enum (blendmode, _("Internal Blend Mode of Glowstick"),
    GeglBlendModeTypeglowstick299, gegl_blend_mode_typeglowstick299,
    GEGL_BLEND_MODE_TYPE_SOFTLIGHT)
enum_end (GeglBlendModeTypeglowstick299)
...
#define GEGL_OP_META
#define GEGL_OP_NAME     glowstick299
#define GEGL_OP_C_SOURCE glowstick299.c
...
update_graph (GeglOperation *operation)
{
  ...
  gegl_node_link_many (state->input,  state->noisereduction, state->gegl1, usethis,  state->lightchroma, state->softglow, state->bloom, state->output,  NULL);
...
GeglProperties *o = GEGL_PROPERTIES (operation);
  GeglNode *input, *output,  *color, *gegl1, *bloom, *linearlight, *hardlight,  *lightchroma, *burn, *multiply, *softglow, *hslcolor, *lchcolor, *overlay, *softlight, *grainmerge, *noisereduction;
...
linearlight = gegl_node_new_child (gegl,
  ...
  gegl_node_link_many (input,  noisereduction,  gegl1,  softlight, lightchroma, bloom, softglow, output, NULL);
...
GeglOperationMetaClass *operation_meta_class = GEGL_OPERATION_META_CLASS (klass);
  ...
  gegl_operation_class_set_keys (operation_class,
    "name",        "lb:glowstick299",
    "title",       _("Glow Stick (no crash 2.99.19)"),
    "reference-hash", "ha3g451fv0nyesyeesg5sgac299",
    "description", _("Makes a image neon like a glow stick (2.99 friendly build)"
                     ""),
    "gimp:menu-path", "<Image>/Colors",
    "gimp:menu-label", _("Glowstick..."),
    NULL);
}
