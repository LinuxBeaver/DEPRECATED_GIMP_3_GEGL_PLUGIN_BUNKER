...
id=2
median-blur radius=0 abyss-policy=none
id=3 over aux=[ ref=3 sg:glassovertext ]
...
hue-chroma hue=49 lightness=-3 sg:metallic guichange=colormetal color=#ffe600 solar1=6.2 ]
...
static void attach (GeglOperation *operation)
{
  ...
  GeglColor *basecolor = gegl_color_new ("#f95dff");
    ...
    GeglNode *unequalblur    = gegl_node_new_child (gegl,
                                  "operation", "gegl:gaussian-blur", "std-dev-x", 3.0, "std-dev-y", 1.0,  "clip-extent", FALSE, "abyss-policy", 0,
                                  NULL);
    ...
    GeglNode *liq    = gegl_node_new_child (gegl,
                                  "operation", "gegl:gaussian-blur", "std-dev-x", 1.0, "std-dev-y", 1.0,  "clip-extent", FALSE, "abyss-policy", 0,
                                  NULL);

#define colorgraph \
" id=1 gimp:layer-mode layer-mode=overlay opacity=0.6 composite-mode=auto aux=[ color value=#ecff0a ] "\

    GeglNode *graphy    = gegl_node_new_child (gegl,
                                  "operation", "gegl:gegl", "string", colorgraph,
                                  NULL);
  ...
  gegl_node_link_many (input, liq, hcolor, unequalblur, idref1, screen, fix1, idref2, fix2, idref3, normal, behind1, hue1, behind2, idref4, luma, bloom, ta, lightweak, extrusion, output, NULL);
...
/*The behind blend mode idref2 with median blur is here*/
  ...
  gegl_node_link_many (idref2, grow2, metallic, graphy, hue2, NULL);
