...
sg:invert-transparency
sg:outlinebevel mode=outline
 */
...
static void attach (GeglOperation *operation)
{
  ...
  state->outlinedeluxe = gegl_node_new_child (gegl,
                                  "operation", "sg:ssg", "radius", 0,
                                  NULL);
  state->inverttrans = gegl_node_new_child (gegl,
                                  "operation", "sg:invert-transparency",
                                  NULL);
...
static void update_graph (GeglOperation *operation)
{
  ...
  if (!state) return;
  GeglNode *vignette = NULL; 
/*
  GeglNode *inverttrans;
*/
  GeglNode *outlinedeluxe = NULL; 
  GeglNode *erase = NULL; 
