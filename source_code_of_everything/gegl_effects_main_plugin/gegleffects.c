...
The fifth is if the namespace gegl: is blocked (for GEGL only) and the namespace sg: gets disallowed for being too short, changing it to a new namespace will fix things for sure.
...
DropShadow is used in place of the hidden operation. REQUIRES sg:bevel and sg:innerglow
...
#id=6 multiply aux=[ ref=6 sg:bevel elevation=49 depth=93  ]
#id=3 over aux=[ ref=3 glassovertext ]
#id=7 id over aux=[ ref=7 sg:innerglow   ]
...
property_int (osdepthextra, _("Extra Outline's Bevel Depth"), 9)
    description (_("Depth of Outline Bevel that works with some but not all blend modes"))
    value_range (1, 40)
  ui_meta ("visible", "guichange {extraosg}")
...
strokenop = gegl_node_new_child (gegl,
 ...
 /* All nodes relating to glass on text end here */
  ...
  gegl_operation_meta_redirect (operation, "depth", thebevel, "depth");
  gegl_operation_meta_redirect (operation, "depth", thebevelsharp, "depth");
  gegl_operation_meta_redirect (operation, "gaus", thebevel, "gaus");
  gegl_operation_meta_redirect (operation, "elevation", thebevelsharp, "elevation");
  gegl_operation_meta_redirect (operation, "elevation", thebevel, "elevation");
...
gegl_op_class_init (GeglOpClass *klass)
{
 ...
 /*btw, if GEGL Effects (or any of my plugins) ever breaks try changing the name space from gegl: or sg: to something else.*/
  gegl_operation_class_set_keys (operation_class,
    "name",        "sg:layereffects",
